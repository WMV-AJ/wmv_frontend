  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (ideaRef.current) {
        ...
        const p = clamp((scroller.clientHeight * .82 - rect.top) / (rect.height + scroller.clientHeight * .25));
        ...
        fragments.forEach((element, index) => {
          ...
          if (!from) return;
          element.style.transform = `translate3d(${from.x * (1 - p)}px,${from.y * (1 - p)}px,0) rotate(${from.r * (1 - p)}deg)`;
        });
      ...
      if (howRef.current) {
        ...
        howRef.current.querySelectorAll<HTMLElement>('[data-flow]').forEach((element, index) => {
          element.style.transform = `translate3d(${(index % 2 ? -1 : 1) * (1 - p) * 18}px,${(1 - p) * 25}px,0)`;
        });
      }
    };
  ...
  return <main ref={scrollerRef} className={`${styles.page} ${motionEnabled ? '' : styles.still}`}>
    ...
    <header className={styles.header}>
      <div className={styles.headerInner}>
        <Link href="/home3" className={styles.brand} aria-label="Where's My Vibe home"><img className={styles.brandLogo} src={logoSrc} alt="" aria-hidden="true" width="44" height="44" /><span>WHERE&apos;S MY VIBE<span className={styles.brandPeriod}>.</span></span></Link>
        ...
        <button type="button" className={styles.menuButton} aria-label={menuOpen ? 'Close menu' : 'Open menu'} aria-expanded={menuOpen} aria-controls="home3-menu" onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X size={22} /> : <Menu size={22} />}</button>
      </div>
      {menuOpen && <nav id="home3-menu" className={styles.mobileNav} aria-label="Mobile page sections"><a href="#idea" onClick={() => setMenuOpen(false)}>THE IDEA</a><a href="#vibes" onClick={() => setMenuOpen(false)}>VIBES</a><a href="#how" onClick={() => setMenuOpen(false)}>HOW IT WORKS</a><a href="#go" onClick={() => setMenuOpen(false)}>EXPLORE THE CITY</a></nav>}
    </header>
    ...
    <section id="go" className={styles.go} aria-labelledby="go-title"><div className={styles.goInner}><div className={styles.goCopy}><SectionIndex number="04" title="YOUR CITY" /><h2 id="go-title">YOUR NEXT<br />GOOD NIGHT<br /><span>STARTS HERE.</span></h2><p>Pick your city. We&apos;ll show you what&apos;s on.</p><div className={styles.cityControls}><label htmlFor="home3-city">YOUR CITY</label><div className={styles.citySelectWrap}><select id="home3-city" value={selectedCity} onChange={(event) => chooseCity(event.target.value)}>{citySlugs.map((slug) => <option key={slug} value={slug}>{getCityConfig(slug).displayName}</option>)}</select><span aria-hidden="true">⌄</span></div><Link className={styles.cityPrimary} href={`/${selectedCity}/map`}>SEE THE MAP <ArrowUpRight size={19} aria-hidden="true" /></Link><Link className={styles.citySecondary} href={`/${selectedCity}/cards`}>BROWSE THE LIST <ArrowRight size={18} aria-hidden="true" /></Link></div><span className={styles.cityNote}>FREE TO EXPLORE / NO ACCOUNT REQUIRED</span></div>
