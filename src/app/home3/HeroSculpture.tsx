...
export default function HeroSculpture({ pose, onReady, onFallback }: Props) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const finishes = [
        new THREE.MeshPhysicalMaterial({ color: 0xbec0c0, metalness: .86, roughness: .23, clearcoat: .45, side: THREE.DoubleSide }),
        new THREE.MeshPhysicalMaterial({ color: 0x676b6e, metalness: .85, roughness: .29, clearcoat: .35, side: THREE.DoubleSide }),
        new THREE.MeshPhysicalMaterial({ color: 0xe4dfd2, metalness: .74, roughness: .24, clearcoat: .56, side: THREE.DoubleSide }),
      ];
      ...
      const rim = new THREE.PointLight(0xe89fca, 13, 18); rim.position.set(3.3, -1.3, 3.8); scene.add(rim);
      const fill = new THREE.PointLight(0x9debf0, 17, 18); fill.position.set(-4, .4, 3); scene.add(fill);
      const glint = new THREE.PointLight(0xe3e88a, 4, 13); glint.position.set(0, -3.1, 2.2); scene.add(glint);
