...
export default function Home() {
  ...
  const statsVibes = todayVenues.length;
  const statsVenues = new Set(todayVenues.map(v => v.venue_id)).size;
  const statsLive = venues.filter(v => isLiveNow(v.event_date, v.event_time, todayStr, dubaiHour)).length;

  // § A — Today's events: live first, then upcoming sorted by start time
  // Deduplicated by venue so each venue appears once
  const storiesData = (() => {
  ...
  // § B — Tonight: events starting 6 PM+, not yet ended, sorted by start time
  const tonightMap = new Map<number, any>();
  todayVenues
    .filter(v => {
      if (!v.event_time) return false;
      const t = v.event_time.trim().toLowerCase();
      if (t === 'all day') return false;
      const parts = v.event_time.split('-').map((p: string) => p.trim());
      const startH = parseTimeHours(parts[0] || '');
      if (startH === null || startH < 18) return false; // must start 6 PM or later
      // Check if already ended
      const endPart = (parts[1] || '').toLowerCase();
      if (endPart.includes('late') || endPart.includes('sunrise')) return true;
      const endHRaw = parseTimeHours(parts[1] || '');
      if (endHRaw === null) return true;
      const endH = endHRaw < startH ? endHRaw + 24 : endHRaw;
      const nowH = dubaiHour < startH ? dubaiHour + 24 : dubaiHour;
      return nowH < endH;
    })
    .sort((a: any, b: any) => {
      ...
      // tie-break: higher rating first, then venue_id for full determinism
      const ra = a.rating ?? 0, rb = b.rating ?? 0;
      if (ra !== rb) return rb - ra;
      return (a.venue_id ?? 0) - (b.venue_id ?? 0);
    })
    .forEach((v: any) => {
      if (!tonightMap.has(v.venue_id)) tonightMap.set(v.venue_id, v);
    });
  const tonightEvents = Array.from(tonightMap.values()).slice(0, 4);

  // § D — Weekend: next Fri + Sat events (skip today), dedup by venue_id
  const now = new Date();
  ...
  // ── SKELETON SHIMMER ──────────────────────────────────────────────
  const skeletonStyle = (w: string | number, h: string | number, extra?: React.CSSProperties): React.CSSProperties => ({
    width: w,
    height: h,
  ...
  return (
    ...
    >
      ...
      <div style={{ maxWidth: 430, margin: '0 auto', paddingBottom: 40 }}>
        ...
        {/* Search */}
        {false && <div style={{ padding: '14px 18px 0' }}>
          <div style={{
        ...
        <div style={{ padding: '28px 18px 0' }}>
          ...
          {loading ? (
            ...
            <>
                ...
                return (
                  <div style={{ position: 'relative', aspectRatio: '4/3', overflow: 'hidden', marginBottom: 14, background: `linear-gradient(135deg, #1c1c2a, #0a0a14)` }}>
                    ...
                    <div style={{ position: 'absolute', top: 10, left: 10, display: 'flex', gap: 5 }}>
                      {isLiveNow(e.event_date, e.event_time, todayStr, dubaiHour) && (
              ...
              {tonightEvents.slice(1, 4).map((e, i) => (
                <div key={e.venue_id || i} style={{
