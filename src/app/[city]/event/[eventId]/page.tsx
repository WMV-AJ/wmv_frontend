...
export default function EventDetailPage() {
  ...
  const getShareText = useCallback(() => {
    ...
    if (event.ticket_price) {
      const priceText = formatPrice(event.ticket_price as any, ((event as any).city ?? (params?.city as string))) ?? `${event.ticket_price}`;
      ...
      <main ref={mainRef} className="fixed inset-0 overflow-y-auto" style={{ backgroundColor: '#0a0a14' }}>
        <div style={{ maxWidth: 430, margin: '0 auto', minHeight: '100%', paddingBottom: 40 }}>
          ...
          <div className="relative w-full" style={{ height: '280px' }}>
            ...
            <div className="absolute inset-0" style={{ background: 'linear-gradient(to bottom, rgba(0,0,0,0.1) 0%, transparent 40%, rgba(10,10,20,1) 100%)' }} />
  ...
  return (
    <main ref={mainRef} className="fixed inset-0 overflow-y-auto" style={{ backgroundColor: '#0a0a14' }}>
      ...
      <div className="px-5 -mt-2">
        {/* Event Name */}
        <h1 className="text-2xl font-bold leading-tight mt-4" style={{ color: '#f5f2ed' }}>
          {event.event_name}
        </h1>
          ...
          {event.ticket_price && (
            <div className="flex items-start gap-3">
              ...
              <div className="flex flex-col gap-0.5">
                <span className="text-[15px] text-[#6ee7b7] font-semibold">{formatPrice(event.ticket_price as any, ((event as any).city ?? (params?.city as string)))}</span>
          ...
          {event.website_social && (
            <div className="flex items-center gap-3">
              ...
              <a
                ...
                className="text-[15px] text-[#93c5fd] truncate"
              >
        ...
        {eventCats.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-3">
            {eventCats.map((cat: any, i: number) => (
              <span key={i} className="px-3 py-1 rounded-full text-[13px] text-[#5f5a70] border border-[rgba(255,255,255,0.15)]">
        ...
        {event.confidence_score != null && event.confidence_score > 0 && (
          <div className="mt-5">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-[13px] text-[#5f5a70] font-medium">AI Confidence</span>
              <span
        ...
        {(categories.length > 0 || highlights.length > 0 || atmosphere.length > 0 || event.venue_address || event.venue_phone || event.venue_website) && (
          <div className="mt-8">
            <div className="h-px w-full mb-5" style={{ background: 'rgba(255,255,255,0.08)' }} />
            <h3 className="text-[13px] text-[#5f5a70] uppercase tracking-wider font-semibold mb-4">Venue Details</h3>
            <div className="space-y-3">
              {categories.length > 0 && (
                <div className="flex items-center gap-3">
                  <Tag className="w-[16px] h-[16px] flex-shrink-0" style={{ color: iconColor }} />
                  <span className="text-[14px] text-[#a8a2b8]">{categories.join(', ')}</span>
                </div>
              )}
              {event.venue_address && (
                <div className="flex items-start gap-3">
                  <MapPin className="w-[16px] h-[16px] flex-shrink-0 mt-0.5" style={{ color: iconColor }} />
                  <span className="text-[14px] text-[#a8a2b8]">{event.venue_address}</span>
                </div>
              )}
              {highlights.length > 0 && (
                <div className="flex items-center gap-3">
                  <Star className="w-[16px] h-[16px] flex-shrink-0" style={{ color: iconColor }} />
                  <span className="text-[14px] text-[#a8a2b8]">{highlights.join(', ')}</span>
                </div>
              )}
              {atmosphere.length > 0 && (
                <div className="flex items-center gap-3">
                  <Sparkles className="w-[16px] h-[16px] flex-shrink-0" style={{ color: iconColor }} />
                  <span className="text-[14px] text-[#a8a2b8]">{atmosphere.join(', ')}</span>
                </div>
              )}
              {event.venue_phone && (
                <div className="flex items-center gap-3">
                  <Phone className="w-[16px] h-[16px] flex-shrink-0" style={{ color: iconColor }} />
                  <span className="text-[14px] text-[#a8a2b8]">{event.venue_phone}</span>
                </div>
              )}
              {event.venue_website && (
                <div className="flex items-center gap-3">
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-[14px] text-[#93c5fd] truncate"
                  >
              ...
              {event.venue_rating > 0 && (
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[14px] text-[#a8a2b8]">
                    {event.venue_rating.toFixed(1)}
        ...
        {deals.length === 0 && event.special_offers && !event.special_offers.toLowerCase().includes('no special') && (
          <div className="mt-8">
            <div className="h-px w-full mb-5" style={{ background: 'rgba(255,255,255,0.08)' }} />
            <h3 className="text-[13px] text-[#5f5a70] uppercase tracking-wider font-semibold mb-4">Offers</h3>
            <div className="rounded-xl px-4 py-3" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}>
              <p className="text-[14px] text-[#a8a2b8]">{event.special_offers}</p>
            </div>
          </div>
        )}
        {deals.length > 0 && (
          <div className="mt-8">
            <div className="h-px w-full mb-5" style={{ background: 'rgba(255,255,255,0.08)' }} />
            <h3 className="text-[13px] text-[#5f5a70] uppercase tracking-wider font-semibold mb-4">Deals & Offers</h3>
            <div className="space-y-3">
                ...
                return (
                  <div key={i} className="rounded-xl px-4 py-3" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' }}>
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {deal.timing && <span className="text-[12px] text-[#5f5a70]">{deal.timing}</span>}
                    </div>
                    {deal.description && (
                      <p className="text-[13px] text-[#a8a2b8] mt-1.5 leading-relaxed">{deal.description}</p>
                    )}
                  </div>
                );
              })}
          ...
          <div className="mt-8">
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <span className="text-[14px] text-[#f5f2ed] font-medium">View Original Post</span>
                <p className="text-[12px] text-[#5f5a70] truncate">instagram.com/p/{event.instagram_id}</p>
              </div>
              <ChevronRight className="w-4 h-4 text-[#a8a2b8] flex-shrink-0" />
            </a>
          </div>
        )}
        ...
        {related.length > 0 && (
          <div className="mt-8">
            <div className="h-px w-full mb-5" style={{ background: 'rgba(255,255,255,0.08)' }} />
            <h3 className="text-[15px] text-[#f5f2ed] font-bold mb-4">You May Also Like</h3>
            <div className="space-y-2">
              {related.map((r) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Info */}
                    <div className="flex-1 min-w-0">
                      <p className="text-[12px] text-[#5f5a70]">
                        {formatDateLabel(r.event_date)}{rTime.start ? ` · ${formatTimeClean(rTime.start)}` : ''}
                      </p>
                      <p className="text-[14px] text-[#f5f2ed] font-semibold truncate">{r.event_name}</p>
                      <p className="text-[12px] text-[#5f5a70] truncate">{r.venue_name_original || r.venue_name}</p>
                    </div>
        ...
        >
          ...
          >
            ...
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-[17px] font-bold text-[#f5f2ed]">Share event</h3>
              <button
                onClick={() => setShowShareModal(false)}
                className="w-8 h-8 rounded-full flex items-center justify-center"
                style={{ background: 'rgba(255,255,255,0.08)' }}
              >
                <X className="w-4 h-4 text-[#5f5a70]" />
              </button>
            </div>

            {/* Event Summary */}
            <div className="rounded-xl px-4 py-3 mb-5 space-y-1.5" style={{ background: 'rgba(255,255,255,0.06)' }}>
              <div className="flex items-center gap-2">
                <span className="text-[14px]">📅</span>
                <span className="text-[13px] text-[#a8a2b8]">
                  {formatDateLabel(event.event_date)}{time.start ? ` at ${formatTimeClean(time.start)}` : ''}{time.end ? ` - ${formatTimeClean(time.end)}` : ''}
                </span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[14px]">🎵</span>
                <span className="text-[13px] text-[#f5f2ed] font-semibold">{event.event_name}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-[14px]">📍</span>
                <span className="text-[13px] text-[#a8a2b8]">{venueName}</span>
              </div>
              {event.ticket_price && (
                <div className="flex items-center gap-2">
                  <span className="text-[14px]">🎟</span>
              ...
              <div className="flex items-center gap-2">
                <span className="text-[14px]">🔗</span>
                <span className="text-[12px] text-[#93c5fd] truncate">{typeof window !== 'undefined' ? window.location.href : ''}</span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-[11px] text-[#5f5a70] uppercase tracking-wider font-semibold">Share on</span>
              <div className="flex-1 h-px" style={{ background: 'rgba(255,255,255,0.1)' }} />
            </div>
            ...
            <div className="grid grid-cols-2 gap-2.5">
              ...
              <button
                ...
                style={copiedField === 'text'
                  ...
                  : { background: 'rgba(255,255,255,0.08)', color: '#f5f2ed' }
                }
              >
