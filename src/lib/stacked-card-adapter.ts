...
/**
 * Fold a venue's same-day events into a single card.
 *
 * Stage 3 regularly lands two events at one venue on one date. Amoeba Sports
 * Bar's happy hour and its Premier League screening were extracted from the
 * same Instagram story, so the two cards even carried the identical photo, and
 * the venue block (name, rating, area, picture) was repeated verbatim in the
 * carousel. Measured against the live API on 24 Aug: 201 of 983 Bangalore cards
 * and 75 of 371 Dubai cards were a repeat of a venue-day already on screen.
 *
 * The first card of each venue-day survives and carries the whole group in
 * `sameDayCards`; the card component renders that as a switcher. Input order is
 * preserved, so whatever ordering the caller already applied (distance from the
 * map centre, date) still decides where the merged card lands.
 *
 * A venue-day with one event is returned as the exact same object, with no
 * `sameDayCards` key, so every existing card keeps rendering on precisely the
 * path it did before. A card missing a venue id or a date cannot be grouped
 * safely (an empty id would collide every such venue on one date into a single
 * card), so those are passed through untouched as well.
 */
export function mergeSameVenueDayCards<
  T extends { event: { event_date: string }; venue: { id: string } },
>(cards: T[]): Array<T & { sameDayCards?: T[] }> {
  const groups = new Map<string, T[]>();
  const order: string[] = [];

  cards.forEach((card, index) => {
    // The space prefix cannot occur in a real key, so ungrouped cards never
    // collide with a venue-day group or with each other.
    const key = card.venue.id && card.event.event_date
      ? `${card.venue.id}|${card.event.event_date}`
      : ` ungrouped-${index}`;
  ...
  return order.map((key) => {
    const group = groups.get(key)!;
    return group.length === 1 ? group[0] : { ...group[0], sameDayCards: group };
  });
}
